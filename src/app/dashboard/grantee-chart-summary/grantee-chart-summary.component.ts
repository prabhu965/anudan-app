...
@Component({
    selector: 'app-granteechart-summary',
    templateUrl: './grantee-chart-summary.component.html',
    styleUrls: ['./grantee-chart-summary.component.css']
})
export class GranteeChartSummaryComponent implements OnInit, OnChanges, AfterViewChecked {
    ...
    displayReportsChart() {
        ...
        for (let s of this.selected.summary.summary) {
            labels.push(s.name === 'Due' ? 'Due in next 15 days' : s.name === 'Overdue' ? 'Past due date' : s.name === 'Submitted' ? 'Submitted' : '');
            data.push(s.value);
